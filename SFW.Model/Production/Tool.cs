...
{
    public class Tool : ModelBase, IModuleData
    {
        #region Properties
        ...
        public static List<Tool> GetList(string skuNbr)
        {
            var _tempList = new List<Tool>();
            var _rows = MasterDataSet.Tables[typeof(Tool).Name].Select($"[SkuID] = '{skuNbr}'");
            if (_rows.Length > 0)
            {
        ...
        /// <returns>A list of tool's associated with the Sku</returns>
        public static List<Tool> GetList(string skuNbr, int seq, int facCode)
        {
            var _tempList = new List<Tool>();
            try
            {
        ...
        /// <param name="facCode">Current user facility number</param>
        /// <returns>A list of tool's associated with the Sku</returns>
        public static List<Tool> GetList(string skuNbr, string machine, int facCode)
        {
            try
            {
                var _tempList = new List<Tool>();
                if (MasterDataSet.Tables.Contains(typeof(Tool).Name))
                {
                    var _rows = MasterDataSet.Tables[typeof(Tool).Name].Select($"[ID] LIKE '{skuNbr}|0{facCode}*%' AND [MachineID] = '{machine}'");
                    if (_rows.Length > 0)
                    {
                        foreach (var _row in _rows)
                        {
